...
#function 'scatter' support for crossing each files in Array. 
workflow cross_corr {
...
#this pipeline consist of two tasks; sba1, sba2.
task sba1{
    ...
    command <<<
        ...
        CODE
    >>>
    #output file name going to be '.rna.qc.csv' and '.atac.qc.csv'
    output{
        File out1 = glob("*.rna.qc.csv")[0]
        File out2 = glob("*.atac.qc.csv")[0]
    }

    #VM environment for running task1.
    runtime{
        cpu:2
        memory:"8000MB"
...
task sba2{
    ...
    #VM environments for running task2..
    runtime{
        cpu : 2
        memory : "8000MB"
        disks : "/mnt/disks/sydir 40 SSD"
